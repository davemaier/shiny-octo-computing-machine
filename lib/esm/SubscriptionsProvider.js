...
var SubscriptionsContext = createContext(undefined);
export function useSubscriptions() {
    var context = useContext(SubscriptionsContext);
    if (!context) {
        throw new Error("Missing subscriptions context");
    }
    return context;
}
export function SubscriptionsProvider(_a) {
    var _this = this;
    var children = _a.children, endpointUrl = _a.endpointUrl, token = _a.token;
    var _b = useReducer(subscriptionsReducer, {
        subscriptions: [],
    }), state = _b[0], dispatch = _b[1];
    var ws = useRef();
    var _c = useState(SubscriptionsStatus.CLOSED), status = _c[0], setStatus = _c[1];
    ...
    useEffect(function () {
        if (status === SubscriptionsStatus.CLOSED &&
            token !== "" &&
    ...
    return (React.createElement(SubscriptionsContext.Provider, { value: value }, children));
}
